...
> Если сервер семьи отвечает 5xx или не отвечает вовсе, фронтенд создаёт семью
> в демо-режиме на устройстве и честно пишет об этом. Раньше
> `GET /api/groups/<code>/members/` отдавал 500 (не были применены миграции);
> сейчас для несуществующего кода сервер правильно отвечает 404.
